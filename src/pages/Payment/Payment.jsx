...
export default Payment;
          ...
          {/* ── MIDDLE: Book Grid + Preview + Chapters ── */}
          <div className="payment-col payment-col--mid">
            {isPerChapter ? (
              <>
                {/* Book grid — always visible, filtered by genre selection */}
                {(() => {
                  const displayBooks = selectedGenreFilter
                    ? filteredBooks
                    : selectedBidangId
                  ...
                  return displayBooks.length > 0 ? (
                    <div className="book-grid-list">
                      {displayBooks.map((b) => {
                        const isSelected = String(b.id) === String(selectedBookId);
                        return (
                          <button
                            key={b.id}
                            type="button"
                            className={`book-grid-item${isSelected ? ' book-grid-item--selected' : ''}`}
                            onClick={() => handleBookCardClick(b)}
                          >
                            <div className="book-grid-item__cover-wrap">
                              {b.cover
                              ...
                              {isSelected && <div className="book-grid-item__check">✓</div>}
                            </div>
                            <div className="book-grid-item__info">
                              <div className="book-grid-item__title">{b.title}</div>
                              <div className="book-grid-item__tags">
                    ...
                    <div className="payment-preview-placeholder">
                      <p>{selectedBidangId || selectedGenreFilter ? 'Tidak ada buku untuk filter ini.' : 'Pilih bidang & genre untuk melihat daftar buku.'}</p>
                    </div>
                  );
                })()}

                {/* Detail preview + chapters after book is selected */}
                {selectedBook && (
                  <>
                    <div className="book-preview" style={{ marginTop: '1rem' }}>
                      {selectedBook.cover && (
